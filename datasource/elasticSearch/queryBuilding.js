...
function get_Esquery(metaData, download_type, workbook) {
   var esUrl;
   if (esUrl != null) {
      esUrl = metaData.esUrl;
   }
   else
   ...
   return new Promise((resolve, reject) => {
      ...
      // console.log(resp);
      //metaData = JSON.parse(resp1.hits.hits[0]._source.visualization.visState);
      var status = false;
      var metrics = [];
      var buckets = {};
      var bucket_list = [];
      ...
      for (let i = 0; i < metrics.length; i++) {
         ...
         //console.log("-------------->>", metrics_data.aggs, "+++++++++++++++++=++", body.aggs)
